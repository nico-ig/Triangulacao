...
int Geometry::orientation(const Polygon& poly) {
  ...
  for ( int i = 0; i < 3; i++ ) {
    const Point a = poly[poly.previous(i)];
    const Point b = poly[i];
    const Point c = poly[poly.next(i)];

    product.push_back((b.y-a.y)*(c.x-b.x) - (b.x-a.x)*(c.y-b.y));
  }

  int ori;
  if ( product[0] == product[1] || product[0] == product[2] ) {
    ori = product[0];
  }
  else {
    ori = product[1];
