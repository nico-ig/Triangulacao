...
Edge Triangulation::findDiagonal(const Polygon& poly) {
  ...
  if ( closest_to_leftmost != Point({oo, 0})) {
    diagonal = {pLeftmost, closest_to_leftmost};
  }
  else {
...
int Triangulation::makeTriangulation(const Polygon& poly) {
  Geometry *geometry = Geometry::getInstance();
  ...
  for ( int i = 0; i < 2; i++ ) { 
    split_id[i] = makeTriangulation(split[i]);
  }
  ...
  return findEdgeInNeighbors(T[split_id[0]], diagonal) != -1 ? split_id[0] : split_id[1];
}
